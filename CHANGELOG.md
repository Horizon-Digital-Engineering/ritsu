...
Denial visibility + opt-in escalation approval. Blocked inter-agent calls
used to be invisible to the operator — a real, working security block looked
like a misbehaving agent. Now every `ask_agent` denial is recorded and
surfaced live, and capability escalation can be routed to an operator
decision instead of a flat hard-deny.
