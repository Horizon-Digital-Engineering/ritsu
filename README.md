...
Bind-mounts the host's `~/.claude/` so the `direct` runtime works without an API key. State persists in the `ritsu-data` named volume. Both ports bound to `127.0.0.1` by default.
