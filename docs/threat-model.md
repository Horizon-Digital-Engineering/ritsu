...
- **Secret rotation for the master key.** Possible today (set new key, run a re-encrypt script) but the script doesn't exist yet. Will add when there are real ciphertexts in production needing rotation.
