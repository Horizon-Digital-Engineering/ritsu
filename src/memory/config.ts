 ...
 *
 *   RITSU_MEMORY_BACKEND = sqlite (default) | flashback | dual
 ...
 *
 *   RITSU_FLASHBACK_URL   base URL of the flashback server (required unless sqlite)
 *   RITSU_FLASHBACK_TOKEN bearer token for that server   (required unless sqlite)
 *   RITSU_FLASHBACK_TIMEOUT_MS  per-request timeout, default 5000
 *
 ...
 * a fully-resolved config. Throws only on an unrecognized RITSU_MEMORY_BACKEND;
...
export function loadMemoryConfig(env: NodeJS.ProcessEnv = process.env): MemoryConfig {
  const raw = (env.RITSU_MEMORY_BACKEND ?? 'sqlite').trim().toLowerCase();
  if (raw !== 'sqlite' && raw !== 'flashback' && raw !== 'dual') {
    throw new Error(`RITSU_MEMORY_BACKEND=${raw}: expected one of sqlite|flashback|dual`);
  }
  const mode: MemoryMode = raw;
  if (mode === 'sqlite') return { mode };

  const endpoint = env.RITSU_FLASHBACK_URL?.trim();
  const token = env.RITSU_FLASHBACK_TOKEN?.trim();
  if (!endpoint || !token) {
  ...
  const timeoutMs = Number(env.RITSU_FLASHBACK_TIMEOUT_MS ?? '') || DEFAULT_TIMEOUT_MS;
